...
[![Version](https://img.shields.io/badge/Version-0.2.11-green.svg)](./docs/RELEASE_NOTES_0_2_11.md)
...
- `@bsbofmusic/cdper@latest`（当前 `1.0.10`）
- `@bsbofmusic/cdper-mcp@latest`（当前 `1.5.9`）
- `CDPER_KERNEL=playwright-cdp`
- `cdper --kernel playwright-cdp ...`

不要安装独立 kernel 包，也不要让工具启动本地浏览器；所有操作都必须连接同一个 CDP Bridge WS endpoint。

### 1. 安装 remote-cdp skill
...
### 2. 配置 cdper MCP（Playwright CDP only）
...
如果还没有 CDP 配置，Agent 必须向用户索要 CDP Bridge 生成的连接信息，不要猜 token、不扫公网：
...
cdper MCP 是默认强交互入口；复杂批量脚本可以直接用 `playwright-core` 连接同一个 Bridge：
...
> “不想写重复的 Playwright CDP 对接代码，要能直接用Agent自动化网页”
...
3. Store it in `~/.cdp-auth.json` or `~/.cdp-bridge/config.json` locally, then run `npx -y @bsbofmusic/cdper@latest --kernel playwright-cdp doctor --json` or call the `cdper_doctor` MCP tool with `CDPER_KERNEL=playwright-cdp`.
...
### 1. cdper MCP（Playwright CDP 强交互真实浏览器控制工具）

cdper 是官方配套的 MCP 工具，使用 Playwright CDP 内核对接 CDP Bridge，实现打开页面、snapshot、点击、输入、按键、等待、截图、执行 JS 和 tab 管理。
...
✅ 强交互控制 | ✅ 登录态复用 | ✅ 截图取证 | ✅ Playwright CDP 内核
👉 [cdper MCP文档](https://www.npmjs.com/package/@bsbofmusic/cdper-mcp)

#### 远端浏览器工具分工

远端 Windows Chrome/Edge 是唯一真实登录态浏览器。本仓库只保留 Playwright 体验线：

| 任务 | 推荐工具 | 说明 |
|---|---|---|
| 标准登录态交互 | cdper MCP + `CDPER_KERNEL=playwright-cdp` | 默认 Agent 入口：open/snapshot/click/type/wait/eval/screenshot/close |
| 复杂脚本交互、批量 DOM、视觉闭环截图 | `playwright-core` + `chromium.connectOverCDP` | 连接同一个 CDP Bridge WS；不要 `launch()` 本地浏览器 |

如果任务需要用户浏览器状态，禁止用本地浏览器、独立无登录态浏览器、curl 或 web_search 冒充成功。cdper 失败时先诊断 bridge/token/port；如果 `/json/version?token=<TOKEN>` 可用，可以用 `playwright-core` 连接同一个 WS endpoint 继续高级操作。
...
npm start         # 本地运行
npm run check     # 语法检查
npm run verify:cdper-playwright  # 验证 cdper Playwright CDP latest + 真实 bridge smoke
