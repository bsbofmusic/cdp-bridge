...
function buildGenericAgentPrompt(snapshot) {
  ...
  if (language === 'en-US') {
    return [
      ...
      'Do not use raw local port 9222 and do not ask for firewall or remote-debugging-address changes.',
      '',
      ...
      '',
      '2. If Browser and webSocketDebuggerUrl are already returned, connect directly with the WS endpoint.',
      '',
      '3. If the endpoint returns 503 or says local CDP is not ready, start the mode you need:',
      ...
      '',
      '4. Poll the HTTP endpoint again until webSocketDebuggerUrl becomes available.',
      '',
      ...
      '- Remote start is allowed when the local browser is not ready.',
      ...
      '- whether remote start was used and which mode was started',
  ...
  return [
    ...
    `curl -s "${httpUrl}" --connect-timeout 5`,
    '',
    '2. 如果已经返回 Browser 和 webSocketDebuggerUrl，就直接用 WS 地址连接。',
    '',
    '3. 如果返回 503，或者提示本地 CDP 未就绪，就先远程拉起你需要的模式：',
    ...
    '',
    '5. 只有拿到可用的 webSocketDebuggerUrl 后，才开始真正的 CDP / Playwright 操作。',
    '',
