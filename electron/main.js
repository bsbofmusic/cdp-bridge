...
function buildGenericAgentPrompt(snapshot) {
  ...
  if (language === 'en-US') {
    return [
      ...
      '5. Before opening pages, inspect bridge diagnostics to see existing agent sessions and targets:',
      `curl -s "${diagnosticsUrl}"`,
      '6. Connect with a unique sessionId so the bridge can track your tabs and diagnostics:',
      wsUrlWithSession,
      '7. If you need a clean tab set for your own session, later close only your own created targets:',
      `curl -X POST "${closeSessionTargetsBase}&sessionId=${sessionIdExample}"`,
      '8. Only set viewport if the layout is clearly wrong:',
      viewportCommand,
      '9. Open the target page:',
      `agent-browser --cdp "${wsUrlWithSession}" open "https://example.com"`,
      '10. Verify the connection and current state:',
      `agent-browser --cdp "${wsUrlWithSession}" get url`,
      `agent-browser --cdp "${wsUrlWithSession}" get title`,
      `curl -s "${diagnosticsUrl}"`,
      '11. Read the page structure:',
  ...
  return [
    ...
    '5. 在打开页面前，先查看 bridge 诊断信息，确认已有 agent 会话和页面：',
    `curl -s "${diagnosticsUrl}"`,
    '6. 连接时追加唯一 sessionId，这样 bridge 才能跟踪你当前会话打开的页面：',
    wsUrlWithSession,
    '7. 如果后面需要清理你自己打开的页面，只关闭当前 sessionId 对应的 targets：',
    `curl -X POST "${closeSessionTargetsBase}&sessionId=${sessionIdExample}"`,
    '8. 只有页面明显变窄或布局不对时，再补执行 viewport：',
    viewportCommand,
    '9. 打开目标页面：',
    `agent-browser --cdp "${wsUrlWithSession}" open "https://example.com"`,
    '10. 验证连接并查看当前诊断：',
