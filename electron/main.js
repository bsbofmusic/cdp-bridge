...
function buildGenericAgentPrompt(snapshot) {
  ...
  if (language === 'en-US') {
    return [
      ...
      '- `cdpReady: false` means the bridge is in standby and the browser CDP is not active yet. It does not by itself mean the Windows machine or Tailscale node is offline.',
  ...
  return [
    ...
    '- `cdpReady: false` 只表示 bridge 处于待机状态、浏览器 CDP 还没激活，不能直接等同于 Windows 主机或 Tailscale 节点离线。',
